...
var qm_template = {
    inint: function ()
    {
        ...
        qm_template.getFreightList(pageNum, pageSize);
    },
    getFreightList: function (pageNum, pageSize)
    {
            ...
            if (state == '1')
            {
                ...
                for (var i = 0; i < data.length; i++)
                {
                    var $Tr = '<tr id="' + data[i].freightId + '">' +
                        ...
                        '<td >' +
                        '<a   onclick="qm_template.editdata(this)">编辑</a>' +
                        '<a   onclick="qm_template.removetemplate(this)">删除</a>' +
                        '</td>' +
                        '</tr>';
                    $('#freight').append($Tr)
                }
...
//控制模板编写
    ...
    AddTr: function ()
    {
        var $Tr = '<tr>' +
            ...
            '<td><input class="addinput" /></td>' +
            '<td >' +
            '<a  onclick="qm_template.editdata(this)">保存</a>' +
            '<a  onclick="qm_template.removetemplate(this)">删除</a>' +
            '</td>' +
            '</tr>';
        var inputval = 0;
    ...
    freightShowNum: function (item)
    {
        ...
        // console.log(pageid)
      qm_template.getFreightList(pageresult, pageSize)
    },

    freightNextPage: function (item)
    {
        //翻页
